  ...
  updater.prototype.checkNewVersion = function(cb){
    ...
    function gotManifest(err, req, data){
      ...
      try{
        ...
        this.remoteManifest = data;
      } catch(e){
        return cb(e)
      }

      var newPatch = false;
      if (data.patch && this.manifest && this.manifest.patch) {
        newPatch = semver.gt(data.patch, this.manifest.patch);
      }
      cb(null, semver.gt(data.version, this.manifest.version), newPatch, data);
    }
  };
   ...
   */
  updater.prototype.download = function(cb, newManifest, type){
    var manifest = newManifest || this.remoteManifest;
    var _platform = type || platform;
    if (_platform == 'patch' && platform == 'mac') {
        _platform = 'patch_mac';
    }
    if (manifest.packages[_platform]) {
        url = manifest.packages[_platform].url;
    } else {
        return;
    }
